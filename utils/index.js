...
function pagesToCommands(mapping, pages) {
  const mappingString = Object.keys(mapping).map(key => `${key}=${mapping[key]}`).join(' ');
  let endString;
  return pages.filter(page => !page.remove).reduce((commands, page, i, activePages) => {
...
exports.pagesToCommands = pagesToCommands;
